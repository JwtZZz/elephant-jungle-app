...
export default function ChatPanel({ apiBase, language }) {
  ...
  const runImageOcr = async (file) => {
    if (!file) return
    setOcrState({ name: file.name, status: 'loading' })
    boost()
    ...
    setOcrState({ name: file.name, status: 'ready' })
  }
    ...
    <div className="right-panel">
      ...
      <div className="input-row">
        <div className="input-wrap">
            ...
            <input
              ...
              onChange={async (event) => {
                ...
                try {
                  ...
                  setOcrState({ name: file.name, status: 'error', detail: error.message })
                } finally {
                  event.target.value = ''
                }
              }}
          ...
          {ocrState ? (
            <div className={`ocr-chip ${ocrState.status}`}>
              <span className="ocr-chip-name">{ocrState.name}</span>
              <span className="ocr-chip-status">
                {ocrState.status === 'loading'
                  ? copy.ocrReading
                  : ocrState.status === 'ready'
                    ? copy.ocrReady
                    : `${copy.ocrFailed}: ${ocrState.detail || copy.requestFailed}`}
              </span>
              <button type="button" className="ocr-chip-clear" onClick={() => setOcrState(null)}>
                ×
              </button>
            </div>
          ) : null}
