...
startup_status: dict[str, object] = {
    ...
    "provider_error": "Provider validation has not run yet.",
    "sync_ready": False,
    "sync_error": "Chroma sync has not run yet.",
}
...
def on_startup() -> None:
    ...
    try:
        sync_chroma_index()
        startup_status["sync_ready"] = True
        startup_status["sync_error"] = ""
    except Exception as exc:
        startup_status["sync_ready"] = False
        startup_status["sync_error"] = str(exc)

    try:
        validate_provider_env()
        startup_status["provider_ready"] = True
        startup_status["provider_error"] = ""
    except Exception as exc:
        startup_status["provider_ready"] = False
        startup_status["provider_error"] = str(exc)
        print(f"Provider validation warning: {exc}")
...
def health() -> dict:
    return {
        "ok": True,
        "providers": {
            "ready": startup_status.get("provider_ready", False),
            "error": startup_status.get("provider_error", ""),
        },
        "knowledge_base": {
            "ready": startup_status.get("sync_ready", False),
            "error": startup_status.get("sync_error", ""),
        },
        "redis": redis_status(),
