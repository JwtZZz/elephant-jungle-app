...
from workflows import AUTHORING_GRAPH, WORKFLOW_PRICE_THRESHOLD_EMAIL, build_execution_graph
...
def _empty_state(language: str, user_email: str) -> dict:
    return {
        "language": _language_from(language),
        "user_email": user_email,
        "draft_task": {"workflow_type": WORKFLOW_PRICE_THRESHOLD_EMAIL, "recipient_email": user_email},
        "missing_fields": [],
        "status": "draft",
        "assistant_message": "",
...
def handle_work_assistant_message(*, user: dict, message: str, session_id: str | None, language: str | None) -> dict:
    ...
    if session is None:
        session = create_work_session(
            user_id=user["id"],
            workflow_type=WORKFLOW_PRICE_THRESHOLD_EMAIL,
            status="draft",
    ...
    current_state["message"] = normalized_message

    next_state = AUTHORING_GRAPH.invoke(current_state)
...
def confirm_work_task(*, user: dict, session_id: str) -> dict:
    ...
    task = create_work_task(
        ...
        operator=draft_task["operator"],
        threshold_value=float(draft_task["threshold_value"]),
        threshold_currency=draft_task["threshold_currency"],
...
def _build_work_email(task: dict, price_payload: dict) -> tuple[str, str]:
    ...
    text = (
        ...
        f"Actual price: {actual_price} {price_payload.get('currency', 'USD')}\n"
    ...
    html = (
        ...
        f"<p><strong>Actual price:</strong> {actual_price} {price_payload.get('currency', 'USD')}</p>"
